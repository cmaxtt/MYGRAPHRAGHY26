...
class DeepSeekClient:
    ...
    async def _scrub_pii(self, text: str) -> str:
        ...
        try:
            results = self.analyzer.analyze(text=text, language='en', entities=["PERSON", "PHONE_NUMBER", "EMAIL_ADDRESS", "IP_ADDRESS"])
    ...
    async def get_completion(self, prompt: str, system_prompt: str = "", model: str = None, scrub_pii: bool = True) -> str:
        """
        ...
        try:
            ...
            if content is None:
                logger.warning(f"Start of response content is None. Full response: {response}")
                return ""
            return content
        except Exception as e:
    ...
    async def get_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        if self._local_embed_model is None:
            ...
            if settings.DEEPSEEK_MODEL_EMBED and "sentence-transformers" in settings.DEEPSEEK_MODEL_EMBED:
                 model_name = settings.DEEPSEEK_MODEL_EMBED
            
            logger.info(f"Loading local embedding model: {model_name}...")
            # Run in executor to avoid blocking event loop during load
            loop = asyncio.get_running_loop()
            self._local_embed_model = await loop.run_in_executor(None, SentenceTransformer, model_name)
